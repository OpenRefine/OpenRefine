...
public class LocalGridState implements GridState {
    ...
    public ApproxCount countMatchingRowsApprox(RowFilter filter, long limit) {
        return grid
                ...
                .aggregate(new ApproxCount(0, 0),
                        (ac, tuple) -> new ApproxCount(ac.getProcessed() + 1, ac.getMatched() + (filter.filterRow(tuple.getKey(), tuple.getValue()) ? 1 : 0)),
    ...
    public ApproxCount countMatchingRecordsApprox(RecordFilter filter, long limit) {
        ...
        return records
                ...
                .aggregate(new ApproxCount(0, 0),
                        (ac, tuple) -> new ApproxCount(ac.getProcessed() + 1, ac.getMatched() + (filter.filterRecord(tuple.getValue()) ? 1 : 0)),
                        (ac1, ac2) -> new ApproxCount(ac1.getProcessed() + ac2.getProcessed(), ac1.getMatched() + ac2.getMatched()));
    }
