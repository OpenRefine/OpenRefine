...
public class UnionPLLTests extends PLLTestsBase {
    ...
    public void testUnion() {
        ...
        Assert.assertEquals(first.getPartitionSizes(), Arrays.asList(2L, 2L));
        Assert.assertEquals(second.getPartitionSizes(), Arrays.asList(2L, 2L));

        PLL<Integer> union = first.concatenate(second);

        Assert.assertEquals(union.getPartitionSizes(), Arrays.asList(2L, 2L, 2L, 2L));
