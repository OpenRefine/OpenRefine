...
/**
 * Base class for a tabular exporter which respects the facets applied to the grid.
 * 
...
public abstract class EngineDependentExporter implements WriterExporter {
    ...
    public void export(GridState grid, Properties options, Engine engine, Writer writer) throws IOException {
        ...
        final boolean outputColumnHeaders = jsonOptions == null ? true : JSONUtilities.getBoolean(jsonOptions, "outputColumnHeaders", true);
        final boolean outputEmptyRows = jsonOptions == null ? false : JSONUtilities.getBoolean(jsonOptions, "outputBlankRows", true);
        final int limit = jsonOptions == null ? -1 : JSONUtilities.getInt(jsonOptions, "limit", -1);

        final List<String> columnNames;
        final Map<String, CustomizableTabularExporterUtilities.CellFormatter> columnNameToFormatter = new HashMap<String, CustomizableTabularExporterUtilities.CellFormatter>();

        List<JsonNode> columnOptionArray = jsonOptions == null ? null : JSONUtilities.getArray(jsonOptions, "columns");
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (columnOptions != null) {
                    ...
                    if (name != null) {
                        columnNames.add(name);
                        try {
                            columnNameToFormatter.put(name, ParsingUtilities.mapper.treeToValue(columnOptions,
                                    CustomizableTabularExporterUtilities.ColumnOptions.class));
