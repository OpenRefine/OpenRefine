...
Date.getMonthNumberFromName = function (name) {
  var n = Date.CultureInfo.monthNames; var m = Date.CultureInfo.abbreviatedMonthNames; var s = name.toLowerCase(); for (var i = 0; i < n.length; i++) { if (n[i].toLowerCase() == s || m[i].toLowerCase() == s) { return i } }
  return -1
}; Date.getDayNumberFromName = function (name) {
  var n = Date.CultureInfo.dayNames; var m = Date.CultureInfo.abbreviatedDayNames; var o = Date.CultureInfo.shortestDayNames; var s = name.toLowerCase(); for (var i = 0; i < n.length; i++) { if (n[i].toLowerCase() == s || m[i].toLowerCase() == s) { return i } }
  return -1
  ...
  var n = (dst || false) ? Date.CultureInfo.abbreviatedTimeZoneDST : Date.CultureInfo.abbreviatedTimeZoneStandard; var p; for (p in n) { if (n[p] === offset) { return p } }
  return null
  ...
  var y = this.getFullYear(); var m = this.getMonth(); var d = this.getDate(); var dow = firstDayOfWeek || Date.CultureInfo.firstDayOfWeek; var offset = 7 + 1 - new Date(y, 0, 1).getDay(); if (offset == 8) { offset = 1 }
  var daynum = ((Date.UTC(y, m, d, 0, 0, 0) - Date.UTC(y, 0, 1, 0, 0, 0)) / 86400000) + 1; var w = Math.floor((daynum - offset + 7) / 7); if (w === dow) { y--; var prevOffset = 7 + 1 - new Date(y, 0, 1).getDay(); if (prevOffset == 2 || prevOffset == 8) { w = 53 } else { w = 52 } }
  return w
}; Date.prototype.isDST = function () { console.log('isDST'); return this.toString().match(/(E|C|M|P)(S|D)T/)[2] == 'D' }; Date.prototype.getTimezone = function () { return Date.getTimezoneAbbreviation(this.getUTCOffset, this.isDST()) }; Date.prototype.setTimezoneOffset = function (s) { var here = this.getTimezoneOffset(); var there = Number(s) * -6 / 10; this.addMinutes(there - here); return this }; Date.prototype.setTimezone = function (s) { return this.setTimezoneOffset(Date.getTimezoneOffset(s)) }; Date.prototype.getUTCOffset = function () { var n = this.getTimezoneOffset() * -10 / 6; var r; if (n < 0) { r = (n - 10000).toString(); return r[0] + r.substr(2) } else { r = (n + 10000).toString(); return '+' + r.substr(1) } }; Date.prototype.getDayName = function (abbrev) { return abbrev ? Date.CultureInfo.abbreviatedDayNames[this.getDay()] : Date.CultureInfo.dayNames[this.getDay()] }; Date.prototype.getMonthName = function (abbrev) { return abbrev ? Date.CultureInfo.abbreviatedMonthNames[this.getMonth()] : Date.CultureInfo.monthNames[this.getMonth()] }; Date.prototype._toString = Date.prototype.toString; Date.prototype.toString = function (format) { var self = this; var p = function p (s) { return (s.toString().length == 1) ? '0' + s : s }; return format ? format.replace(/dd?d?d?|MM?M?M?|yy?y?y?|hh?|HH?|mm?|ss?|tt?|zz?z?/g, function (format) { switch (format) { case 'hh':return p(self.getHours() < 13 ? self.getHours() : (self.getHours() - 12)); case 'h':return self.getHours() < 13 ? self.getHours() : (self.getHours() - 12); case 'HH':return p(self.getHours()); case 'H':return self.getHours(); case 'mm':return p(self.getMinutes()); case 'm':return self.getMinutes(); case 'ss':return p(self.getSeconds()); case 's':return self.getSeconds(); case 'yyyy':return self.getFullYear(); case 'yy':return self.getFullYear().toString().substring(2, 4); case 'dddd':return self.getDayName(); case 'ddd':return self.getDayName(true); case 'dd':return p(self.getDate()); case 'd':return self.getDate().toString(); case 'MMMM':return self.getMonthName(); case 'MMM':return self.getMonthName(true); case 'MM':return p((self.getMonth() + 1)); case 'M':return self.getMonth() + 1; case 't':return self.getHours() < 12 ? Date.CultureInfo.amDesignator.substring(0, 1) : Date.CultureInfo.pmDesignator.substring(0, 1); case 'tt':return self.getHours() < 12 ? Date.CultureInfo.amDesignator : Date.CultureInfo.pmDesignator; case 'zzz':case 'zz':case 'z':return '' } }) : this._toString() }
Date.now = function () { return new Date() }; Date.today = function () { return Date.now().clearTime() }; Date.prototype._orient = +1; Date.prototype.next = function () { this._orient = +1; return this }; Date.prototype.last = Date.prototype.prev = Date.prototype.previous = function () { this._orient = -1; return this }; Date.prototype._is = false; Date.prototype.is = function () { this._is = true; return this }; Number.prototype._dateElement = 'day'; Number.prototype.fromNow = function () { var c = {}; c[this._dateElement] = this; return Date.now().add(c) }; Number.prototype.ago = function () { var c = {}; c[this._dateElement] = this * -1; return Date.now().add(c) }; (function () {
  var $D = Date.prototype; var $N = Number.prototype; var dx = ('sunday monday tuesday wednesday thursday friday saturday').split(/\s/); var mx = ('january february march april may june july august september october november december').split(/\s/); var px = ('Millisecond Second Minute Hour Day Week Month Year').split(/\s/); var de; var df = function (n) {
    return function () {
...
(function () {
  Date.Parsing = { Exception: function (s) { this.message = "Parse error at '" + s.substring(0, 10) + " ...'" } }; var $P = Date.Parsing; var _ = $P.Operators = {
    ...
    until: function (p) {
      return function (s) {
        var qx = []; var rx = null; while (s.length) {
    ...
    many: function (p) {
      return function (s) {
        var rx = []; var r = null; while (s.length) {
    ...
    product: function () {
      var px = arguments[0]; var qx = Array.prototype.slice.call(arguments, 1); var rx = []; for (var i = 0; i < px.length; i++) { rx.push(_.each(px[i], qx)) }
      return rx
    },
    cache: function (rule) {
      var cache = {}; var r = null; return function (s) {
    ...
    each: function () {
      var px = arguments; return function (s) {
        var rx = []; var r = null; for (var i = 0; i < px.length; i++) {
          if (px[i] == null) { continue }
    ...
    all: function () { var px = arguments; var _ = _; return _.each(_.optional(px)) },
    sequence: function (px, d, c) {
      d = d || _.rtoken(/^\s*/); c = c || null; if (px.length == 1) { return px[0] }
      return function (s) {
        var r = null; var q = null; var rx = []; for (var i = 0; i < px.length; i++) {
    ...
    set: function (px, d, c) {
      d = d || _.rtoken(/^\s*/); c = c || null; return function (s) {
        var r = null; var p = null; var q = null; var rx = null; var best = [[], s]; var last = false; for (var i = 0; i < px.length; i++) {
    ...
    return function () {
      var args = null; var rx = []; if (arguments.length > 1) { args = Array.prototype.slice.call(arguments) } else if (arguments[0] instanceof Array) { args = arguments[0] }
  ...
  }; var _ = Date.Parsing.Operators; var g = Date.Grammar; var t = Date.Translator; var _fn; g.datePartDelimiter = _.rtoken(/^([\s\-\.\,\/\x27]+)/); g.timePartDelimiter = _.stoken(':'); g.whiteSpace = _.rtoken(/^\s*/); g.generalDelimiter = _.rtoken(/^(([\s\,]|at|on)+)/); var _C = {}; g.ctoken = function (keys) {
    var fn = _C[keys]; if (!fn) {
      var c = Date.CultureInfo.regexPatterns; var kx = keys.split(/\s+/); var px = []; for (var i = 0; i < kx.length; i++) { px.push(_.replace(_.rtoken(c[kx[i]]), kx[i])) }
