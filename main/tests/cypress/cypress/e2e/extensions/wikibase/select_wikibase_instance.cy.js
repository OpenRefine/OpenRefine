...
describe.skip(__filename, function () {
  ...
  it('Add a wikibase instance, general navigation', function () {
    ...
    cy.get('.wikibase-dialog .dialog-header').should('to.contain', 'Manage Wikibase instances');

    // click add
    cy.get('.wikibase-dialog .dialog-footer button').contains('Add Wikibase').click();

    // check panel
    cy.get('.add-wikibase-dialog .dialog-header').should('to.contain', 'Add Wikibase manifest');
  });
  ...
  it('Add a wikibase instance (JSON Manifest copy-pasted in the textarea)', function () {
    ...
    const manifest = {
      ...
      oauth: {
        registration_page: 'https://or-wikibase-test.wiki.opencura.com/wiki/Special:OAuthConsumerRegistration/propose',
      },
      reconciliation: {
        endpoint: 'https://or-wikibase-test.reconci.link/${lang}/api',
      },
    };
    cy.get('.add-wikibase-dialog textarea').invoke('val', JSON.stringify(manifest));
    cy.get('.add-wikibase-dialog button').contains('Add Wikibase').click();
      ...
      .then((response) => {
        ...
        if (savedValue && Array.isArray(savedValue)) {
          savedValue = savedValue.filter((object) => {
            return object.mediawiki.name !== WIKIBASE_TEST_NAME && object.mediawiki.name !== WIKIBASE_TEST_NAME2;
          });
        }
      });
  }
