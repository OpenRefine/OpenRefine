describe(__filename, () => {
  describe('All menu', () => {
    ...
    it('contains a item called "Add blank rows"', () => {
      ...
      cy.get('div.menu-container a.menu-item').eq(3).invoke('text').should('equal', 'Add blank rows');
    });

    describe('"Add blank rows" submenu', () => {
      it('contains a sub-item called "Prepend one row"', () => {
        cy.columnActionClick('All', ['Add blank rows']);
        cy.get('div.menu-container').eq(1).find('a.menu-item').eq(0).invoke('text').should('equal', 'Prepend one row');
      });

      it('contains a sub-item called "Append one rows"', () => {
        cy.columnActionClick('All', ['Add blank rows']);
        cy.get('div.menu-container').eq(1).find('a.menu-item').eq(1).invoke('text').should('equal', 'Append one row');
      });
  ...
  describe('Peripheral interface', () => {
    function addRows() {
      ...
      // The add-row action has no stable request alias exposed by this helper.
    ...
    it('displays a notification message', () => {
      cy.get('div#notification > div[bind=undoDiv] > span[bind=undoDescription]').as('notificationDescription');

      // Verify default state
      cy.get('@notificationDescription').invoke('text').should('equal', '');

      addRows();

      cy.get('@notificationDescription').invoke('text').should('equal', 'Add 1 row');
    });

    it('increments history count by 1', () => {
      cy.get('a#or-proj-undoRedo > span.count').as('historyCount');

      // Verify default state
      cy.get('@historyCount').invoke('text').should('equal', '0 / 0');

      addRows();

      cy.get('@historyCount').invoke('text').should('equal', '1 / 1');
    });

    it('increments row count summary by 1', () => {
      cy.get('#summary-bar > span').as('summaryText');

      // Verify default state
      cy.get('@summaryText').invoke('text').should('equal', '199 rows');

      addRows();

      cy.get('@summaryText').invoke('text').should('equal', '200 rows');
    });

    it('increments total history entries by 1', () => {
      // Verify default state
      cy.get('a.history-entry').should('have.length', 0);

      addRows();

      cy.get('a.history-entry').should('have.length', 2);
    });

    it('updates the UI when faceting by blank rows', () => {
      cy.columnActionClick('All', ['Facet', 'Facet by blank (null or empty string)']);

      cy.get('li#facet-0 div.facet-body-inner div.facet-choice').as('facetChoices');
  ...
  describe('Modal dialog', () => {
    beforeEach(() => {
      ...
      cy.get('div[data-cy=add-rows-dialog]').as('dialog');
      cy.get('@dialog').find('button[data-cy=cancel-button]').as('cancelButton');
      cy.get('@dialog').find('button[data-cy=submit-button]').as('submitInput');
      cy.get('@dialog').find('input[data-cy=row-count]').as('countInput');
      cy.get('@dialog').find('form[data-cy=add-rows-form]').as('form');
      cy.get('@dialog').find('select[data-cy=insert-position]').as('positionSelect');
    });
  ...
  describe('Data table', () => {
    describe('Prepended blank row', () => {
      beforeEach(() => {
        ...
        cy.get('div#view-panel > div.data-table-container > table > tbody > tr').as('tableRows');
    ...
    describe('Appended blank row', () => {
      ...
      function assertRowIsBlank($tr) {
        $tr.find('td > div.data-table-cell-content > div > span').each((i, span) => {
      ...
      describe('In row mode', () => {
        ...
        it('is the last row in the project', () => {
          cy.get('div#view-panel div.viewpanel-pagesize a.viewPanel-pagingControls-page.action').last().click();
          cy.get('@tableRows').last().then(assertRowIsBlank);
        });
