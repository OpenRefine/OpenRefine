...
describe(__filename, function () {
  ...
  it('Transpose cells across columns into rows (One column)', function () {
    ...
    cy.columnActionClick('Street', ['Transpose', 'Transpose cells across columns into rows…']);

    cy.get('.dialog-container select[bind="fromColumnSelect"]').select('Street');
    cy.get('.dialog-container select[bind="toColumnSelect"]').select('(last column)');

    // add the column name and "test" string to each transposed cell
    cy.get('label').contains("prepend the original column's name to each cell").click();
    cy.get('input[bind="separatorInput"]').type(' test ');
    cy.get('.dialog-container label').contains('One column').click();
    cy.get('.dialog-container label').contains('One column').parent().find('input').type('Address');

    cy.confirmDialogPanel();
    cy.assertNotificationContainingText('Transpose cells in columns starting with Street');
  ...
  it('Transpose cells across columns into rows (Two columns)', function () {
    ...
    cy.get('.dialog-container input[bind="keyColumnNameInput"]').type('Address part');
    cy.get('.dialog-container input[bind="valueColumnNameInput"]').type('Address');
