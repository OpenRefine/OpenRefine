describe('Create new item for each cell', () => {
  ...
  it('Test mark to create new items in many cells, previously reconciled', () => {
    ...
    cy.columnActionClick('species', ['Reconcile', 'Actions', 'Create a new item for each cell']);

    // check notification and ensure no rows it matched anymore
    cy.assertNotificationContainingText(
      'Mark to create new items for 6 cells in column species, one item for each cell'
    );
  ...
  it('Test mark to create new items in many cells, previously reconciled 2', () => {
    ...
    cy.columnActionClick('identifier', ['Reconcile', 'Actions', 'Create a new item for each cell']);
