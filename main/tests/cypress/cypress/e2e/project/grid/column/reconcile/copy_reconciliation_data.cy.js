describe('Copy reconciliation data', () => {
  ...
  it('Copy reconciliation data from species to species_copy', () => {
    ...
    cy.columnActionClick('species', ['Edit column', 'Add column based on this column…']);
    cy.waitForDialogPanel();
    cy.get('input[bind="columnNameInput"]').type('duplicated_column');
    cy.confirmDialogPanel();

    cy.columnActionClick('species', ['Reconcile', 'Copy reconciliation data']);

    // check the dialog, enter a new column name "id_column"
    cy.get('.dialog-container .dialog-header').should('to.contain', 'Copy recon judgments from column species');
    cy.get('.dialog-container select[bind="toColumnSelect"]').select('duplicated_column');

    cy.get('.dialog-container .dialog-footer button').contains('Copy').click();

    cy.assertNotificationContainingText('Copy 4 recon judgments from column species to duplicated_column');

    cy.get('table.data-table th')
      .contains('duplicated_column')
      .should('exist')
      .then(() => {
