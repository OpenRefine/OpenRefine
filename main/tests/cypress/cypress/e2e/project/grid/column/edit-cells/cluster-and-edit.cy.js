...
describe(__filename, function () {
  it('Test Layout, Popup & Close', function () {
    ...
    cy.get('.dialog-container').within(() => {
      ...
      cy.get('.dialog-body').contains('Find groups of different cell values');
      cy.get('.dialog-footer').contains('Export clusters');
    });

    cy.get('.dialog-container .dialog-footer button[bind="closeButton"]').contains('Close').click();
  ...
  it('Test the footer buttons, Select/Unselect All', function () {
    ...
    cy.get('.dialog-container').within(() => {
      ...
      cy.get('.clustering-dialog-entry-table input[type="checkbox"]').should('be.checked');

      cy.get('.dialog-footer button[bind="deselectAllButton"]').click();
      cy.get('.clustering-dialog-entry-table input[type="checkbox"]').should('not.be.checked');
    });
  });
  ...
  it('Test the clustering using custom function', function () {
    ...
    cy.get('.dialog-container').within(() => {
      ...
      cy.get('.clustering-dialog-entry-table tr.odd input[type="checkbox"]').check();

      // enter a new cell value
      cy.get('.clustering-dialog-entry-table tr.odd input[type="text"]').type('testing');
  ...
  it('Test the different clustering options for rendering and expected inputs', function () {
    ...
    cy.get('.dialog-container').within(() => {
      cy.get('.clustering-dialog-entry-table').should('to.exist').should('be.visible');

      // select a method
      for (const method of methods) {
        console.log(method);
        cy.get('.clustering-dialog-entry-table').should('to.exist').should('be.visible');
        cy.get(`.dialog-body select[bind="methodSelector"]`).select(method['methodName']);
        ...
        for (const functionName of method['functions']) {
          ...
          cy.get('.clustering-dialog-entry-table').should('to.contain', 'BALLARDS');
        }
      }
    });
  });
  ...
  it('Merge Select & Re Cluster', function () {
    ...
    cy.get('.dialog-container').within(() => {
      ...
      // enter a new cell value
      cy.get('.clustering-dialog-entry-table tr.odd input[type="text"]').type(' TESTING A');
      ...
      // enter a new cell value
      cy.get('.clustering-dialog-entry-table tr.odd input[type="text"]').type(' TESTING B');

      cy.get('.dialog-footer button[bind="applyReClusterButton"]').click();

      // Close

      cy.get('.dialog-footer button[bind="closeButton"]').contains('Close').click();
    });
  ...
  it('Ensure duplicate whitespace are preserved, #4004', function () {
    ...
    cy.get('.dialog-container').within(() => {
      cy.get('.clustering-dialog-entry-table tr td:nth-child(2)').should('to.contain', 'b ar');
      cy.get('.clustering-dialog-entry-table tr td:nth-child(2)').should('to.contain', 'b  ar');
      cy.get('.clustering-dialog-entry-table tr td:nth-child(2)').should('to.contain', 'b   ar');
    });
  });
