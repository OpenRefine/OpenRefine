describe(__filename, function () {
  ...
  it('Ensure the Edit button opens a popup', function () {
    cy.loadAndVisitProject('food.mini');
    cy.getCell(1, 'Shrt_Desc').as('cell');
    cy.get('@cell').trigger('mouseover');
    cy.get('@cell').find('.data-table-cell-edit').click();
  ...
  it('Test a simple edit, using keyboard shortcut', function () {
    ...
    cy.get('@cell').find('.data-table-cell-edit').click();
    cy.get('.menu-container.data-table-cell-editor').should('exist');
    cy.get('.menu-container.data-table-cell-editor textarea').as('textarea');
    cy.get('@textarea').type('OpenRefine Testing');
