...
describe(__filename, function () {
  it('Test select/deselect all', function () {
    ...
    cy.assertTextareaHaveJsonValue('.dialog-container textarea.history-operation-json', defaultInducedHistoryJson);
  });
  ...
  it('Test download of JSON file', { retries: { runMode: 0 } }, function () {
    ...
    cy.get('.dialog-container button[bind="saveJsonAsFileButton"]').contains('Export').click();

    const downloadsFolder = Cypress.config('downloadsFolder');
    const downloadedFilename = downloadsFolder + '/history.json';

    cy.readFile(downloadedFilename, 'utf8').should('deep.equal', defaultInducedHistoryJson);
  });
