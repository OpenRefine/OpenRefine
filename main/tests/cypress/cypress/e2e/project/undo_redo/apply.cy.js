describe(__filename, function () {
  it('Apply a recipe', function () {
    ...
    cy.get('table.data-table thead th[title="Energ_Kcal"]').should('not.to.exist');

    cy.assertNotificationContainingText('3 operations applied');
    cy.get('a#or-proj-undoRedo > span.count').invoke('text').should('equal', '3 / 3');
    cy.get('#summary-bar > span').invoke('text').should('equal', '5 rows');

    cy.get('.notification-action a').should('to.contain', 'Undo').click();

    cy.get('a#or-proj-undoRedo > span.count').invoke('text').should('equal', '0 / 3');
  });
  ...
  it('Apply a recipe that does not reference any column', function () {
    ...
    cy.get('.data-table tr:nth-child(1) td:nth-child(2) a').should('have.class', 'data-table-flag-off').click();
  ...
  it('Use an invalid JSON payload', function () {
    ...
    cy.get('.dialog-container .history-operation-json').type('[{foo', {
