describe(__filename, function () {
  ...
  it('Test the create project from this computer based on CSV', function () {
    ...
    cy.get('.create-project-ui-source-selection-tab-body.selected input[type="file"]').selectFile(
      'cypress/fixtures/food.mini.csv'
    );
  ...
  it('Test the create project from this computer based on TSV', function () {
    ...
    // add file
    cy.get('.create-project-ui-source-selection-tab-body.selected input[type="file"]').selectFile(
      'cypress/fixtures/shop.mini.tsv'
    );
    cy.get('.create-project-ui-source-selection-tab-body.selected button.button-primary').contains('Next »').click();
    cy.get('.default-importing-wizard-header input[bind="projectNameInput"]', {
      timeout: 6000,
  ...
  it('Test the create project from Web URL based on CSV', function () {
    cy.visitOpenRefine();
    cy.navigateTo('Create project');
    cy.get('#create-project-ui-source-selection-tabs > a').contains('Web Addresses (URLs)').click();
  ...
  it('Test the create project from Multiple Web URLs based on CSV', function () {
    ...
    // then ensure we are on the preview page
    cy.get('.default-importing-wizard-header input[bind="projectNameInput"]').should('have.value', 'food mini 2 csv');
  ...
  it('Test the create project from XML file after selecting record path', function () {
    ...
    // add file
    cy.get('.create-project-ui-source-selection-tab-body.selected input[type="file"]').selectFile(
      'cypress/fixtures/AAT-with-xsi.xml'
    );
    cy.get('.create-project-ui-source-selection-tab-body.selected button.button-primary').contains('Next »').click();
    cy.get('.default-importing-wizard-header input[bind="projectNameInput"]', {
      timeout: 6000,
    ...
    cy.get(
      'div.xml-parser-ui-select-dom > div > div.children > div.elmt > div.children > div:nth-child(14) > div.children > div:nth-child(2) > div:nth-child(1)'
    )
      .contains('Preferred_Term')
      .click();

    // Verify that the first cell in our grid has the right contents
    cy.get('div.default-importing-parsing-data-panel > table > tbody > tr.odd > td:nth-child(2) > div > span').contains(
