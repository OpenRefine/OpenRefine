...
/**
 * Stores statistics related to time values stored in a set of rows (generated by an expression). The time values are
 * represented as milliseconds since Epoch, and stored individually in an array. This is different from the numeric
 * facet which does the binning on the fly, because the various possible scales for the timeline facet do not align (for
 * instance, years are not integer multiples of days).
 * 
...
public class TimeRangeStatistics implements Serializable {
    ...
    /**
     * The array of time values might be longer than the number of time values actually stored - see
     * {@link getTimeCount()} to get the actual number of values stored.
     */
