 ...
 * is used to determine the bin size, called the global histogram. This is added to ensure that the bin size does not
 * change when interactively faceting.
 * 
...
public class NumericFacetState implements FacetState {
    ...
    /**
     * To be called after aggregating on all rows, to force a single-valued histogram to be represented as a single bin,
     * with the supplied log bin size. It also deals with the case where no numeric value was present in the view, but
     * some was found outside the view, in which case the view histogram should
     * 
     * @return
     */
    public NumericFacetState normalizeForReporting(int logBinSize) {
        ...
        if (_global.getNumericCount() > 0 && _view.getNumericCount() == 0) {
            ...
            if (globalHistogram.getMinBin() < viewHistogram.getMinBin() ||
                ...
                rescaled = new NumericFacetState(globalHistogram,
                        viewHistogram.extend(globalHistogram.getMinBin(), globalHistogram.getMaxBin()));
            }
            return rescaled;
        }
    }
