...
/**
 * Stores a histogram of numerical values, used in range facets. The histogram is computed using a uniform subdivision
 * of the interval in which the distribution spreads and the bin size is required to be an integer power of the bin base
 * (10 by default).
 * 
...
public class HistogramState implements Serializable {
     ...
     *            number of non numeric values (text, dates…)
     * @param errorCount
     *            number of error values
     * @param blankCount
     *            number of blank values
     * @param logBinSize
     *            size of a bin, as an exponent of the base (10)
     * @param minBin
     *            lowest boundary of the lowest bin, divided by the bin size
     * @param bins
     *            array of bins, each of which contains the number of values in that bin
     */
    ...
    /**
     * The start of the first bin, represented as an integer. To get the actual value (as a double), multiply by the bin
     * size.
     */
    ...
    /**
     * The end of the last bin, represented as an integer. To get the actual value (as a double), multiply by the bin
     * size.
     */
    ...
    /**
     * Given a larger bin size (therefore generating coarser bins), return a new version of this facet state by merging
     * the neighbouring bins together to obtain the desired bin size.
     * 
     * @param newLogBinSize
     *            the new power of 10 to use as a bin size
     * @return
     */
    ...
    @Override
    public int hashCode() {
        return Long.hashCode(_numericCount) + 17 * Long.hashCode(_nonNumericCount) + 5 * Long.hashCode(_errorCount)
                + 23 * Long.hashCode(_blankCount);
    }
