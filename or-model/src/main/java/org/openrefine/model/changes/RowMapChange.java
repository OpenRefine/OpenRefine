 ...
 * 
 * This class can also be subclassed by changes which ignore the engine,
...
public abstract class RowMapChange extends EngineDependentChange {
	...
	/**
	 * Stores the new column model to be applied on the new grid,
	 * and the row mappers used to derive the new grid.
	 * 
	 * The positive mapper is applied to the rows matched by the filter,
	 * the negative one is applied to the other rows.
	 * 
	 * Computing both the ColumnModel and the RowMappers can be useful
	 * in certain situations (such as {@link ColumnSplitChange}) where
	...
	/**
	 * Returns the function that is applied to each row and row index, the new
	 * column model applied to the grid, and the new overlay models.
	 * Computing these in one go can be useful in certain situations (such as {@link ColumnSplitChange}).
	 * 
	 * If a subclass needs to return only one non-default value out of
	 * mapper, column model and overlay models it can instead override the corresponding
	 * method.
	 * 
	 * @param state the initial column model
	 * @throws DoesNotApplyException if the change does not apply to the given grid
	 */
	...
	/**
	 * Returns the column model after the change is applied to the given grid state.
	 * By default, returns the same column model.
	 * 
	 * @param state the grid to which the change should be applied
	...
	/**
	 * Method run at the end of the change to update any statistics (such
	 * as ReconStats or overlay models) on the resulting state.
	 * @param gridState the grid after the map operation
	...
	/**
	 * Utility method to retrieve a column index and throw an exception
