...
/**
 * A RDD of Records which is efficiently computed from the corresponding
 * RDD of indexed rows. Rows are grouped into records, even when the records
 * cross partition boundaries. This grouping is also stable: turning a RDD of records into
 * a RDD of rows with the same partitioner, and then grouping the rows into records again gives the 
 * same partitioner. This last property ensures that there is no drift of rows across partitions
 * as the grid gets grouped/ungrouped into records repeatedly.
 * 
...
public class RecordRDD extends RDD<Tuple2<Long,Record>> implements Serializable {
    ...
    /**
     * A partition holds a pointer to the source partition, and a list of additional rows
     * which should be added at the end of the partition to complete the last record which
     ...
     * 
     * @param prev
     *     the RDD of indexed rows, where row ids are assumed to be
    ...
    /**
     * Extracts the first rows of a partition until a row with a non-blank value in the record
     * key column is encountered. This can fetch the entire partition if no record key is found.
     * 
     * @author Antonin Delpeuch
     *
     */
