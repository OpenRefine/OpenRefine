...
:::caution
Note that some syntax is different in this tool than elsewhere in OpenRefine: a forward slash must be escaped with a backslash, while other characters do not need escaping. You cannot, at this time, include doubled closing curly brace (}}) anywhere in your expression, or it will cause it to malfunction.
:::
