...
### DeflateNCDBenchmark (NCD distance comparison)

Class:

- `org.openrefine.benchmark.DeflateNCDBenchmark`

Methods:

- `deflateShortStrings` / `vicinoPPMShortStrings`
- `deflateMediumStrings` / `vicinoPPMMediumStrings`
- `deflateLongStrings` / `vicinoPPMLongStrings`

Purpose:

- Compare per-call distance computation performance between the new Deflate-based NCD implementation and Vicino's PPM arithmetic coding distance across three string-length categories (short ~8 chars, medium ~34 chars, long ~57 chars).

Examples:

```bash
# Run full comparison (all six methods)
java -jar benchmark/target/openrefine-benchmarks.jar DeflateNCDBenchmark

# Run only the Deflate methods
java -jar benchmark/target/openrefine-benchmarks.jar DeflateNCDBenchmark.deflate

# Run only the Vicino PPM methods
java -jar benchmark/target/openrefine-benchmarks.jar DeflateNCDBenchmark.vicino

# Run one specific comparison pair (short strings)
java -jar benchmark/target/openrefine-benchmarks.jar "DeflateNCDBenchmark.(deflateShort|vicinoPPMShort)"

# Quick single-fork run
java -jar benchmark/target/openrefine-benchmarks.jar DeflateNCDBenchmark -f 1
```
